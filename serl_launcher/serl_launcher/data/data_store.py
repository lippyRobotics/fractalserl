...
class FractalSymmetryReplayBufferDataStore(FractalSymmetryReplayBuffer, DataStoreBase):
    def __init__(
        ...
        **kwargs: dict,
        ...
        """Initialize a thread-safe fractal datastore.

        ``front_M`` and ``world_fixed_img_keys`` are explicit parameters so
        homography configuration cannot be hidden inside method-specific
        ``kwargs``. They remain disabled when the matrix is ``None`` and the
        key collection is empty.
        """

        FractalSymmetryReplayBuffer.__init__(
            self,
            observation_space,
            action_space,
            capacity,
