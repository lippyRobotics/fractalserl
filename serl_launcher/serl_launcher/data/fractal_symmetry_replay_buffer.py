...
class FractalSymmetryReplayBuffer(ReplayBuffer):
    ...
    def generate_transform_deltas(self):
        ...
        # Translations
        divisor = total_branches / self.current_branch_count
        x_deltas, y_deltas = np.divmod(idx, divisor)

        divisor = divisor / self.current_branch_count
        y_deltas, z_deltas = np.divmod(y_deltas, divisor)
